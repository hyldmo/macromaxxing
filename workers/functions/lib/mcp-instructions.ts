...
- Pass \`verbose: false\` on workout_getWorkout / listWorkouts / getSession / listSessions to omit nested muscle/equipment lists (~80% smaller). On listSessions it additionally returns \`logs: []\` — the per-session \`summary\` (per-exercise sets / hardSets / volumeKg / topSet) replaces them; call workout_getSession for individual sets.
...
- workout_listSessions is ordered most-recent-first; at \`verbose: true\` it carries every set row, so scan with verbose:false + window/filters and read the \`summary\` rollup.
