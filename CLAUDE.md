...
- `worker-configuration.d.ts` is committed (generated locally where `.dev.vars` exists; CI types depend on it). Excluded from biome via `files.includes` negation. `generate:wrangler` is gated on `.dev.vars` existing (`test -f .dev.vars && wrangler types || true`) so it refreshes locally during `yarn check`/`yarn build` but silently skips in CI — running `wrangler types` in CI would overwrite the file with an empty `Env` and break every route that reads a secret.
