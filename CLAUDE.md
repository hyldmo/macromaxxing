...
- `worker-configuration.d.ts` is committed (generated locally where `.dev.vars` exists; CI types depend on it). Excluded from biome via `files.includes` negation. **Never put `wrangler types` in any script that runs in CI** (e.g. inside `yarn generate`/`typecheck`) — without `.dev.vars` it overwrites the file with an empty `Env` and breaks every route that reads a secret. `yarn codegen` runs it manually for when `wrangler.toml` changes.
