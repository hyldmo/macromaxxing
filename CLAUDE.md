  ...
  features/                                 # Domain helpers: sub-components, hooks, utils. Single-URL page
    ...
    plans/                                  # Cross-domain (meals + workouts) surfaces for /plans
      ...
      utils/weekCalendar.ts                 # Pure: buildWeekDays folds undated meal-plan slots (dayOfWeek) and dated
                                            ...
                                            #   recovery debt: planned days priced from the template
                                            #   (computeProgramRest), trained days from their logged working sets
                                            #   (recoveryHoursFromPriorSession). One pass, no repeats.
    workouts/
...
- **`web-push`'s `sendNotification` cannot run on workerd.** It posts through `node:https`, which answers
  `[unenv] https.request is not implemented yet`, so every send throws and the catch reads it back as a
  provider rejection. The library's crypto is fine there, so `generateRequestDetails` builds the encrypted
  body plus VAPID headers and `fetch` puts them on the wire (`workers/functions/lib/web-push.ts`). Unit tests
  pass either way because they stub the transport; only a real device shows the failure.
...
- **Bottom-anchored `position: fixed` chrome needs `transform-gpu` on iOS 26.** Safari 26 lays such an element
  ...
  - **`viewport-fit=cover` + `env(safe-area-inset-*)` padding was also tried and reverted — it moves the tab
    bar UP.** The reasoning looks sound (cover makes the viewport full-bleed, so `pb-[env(...)]` lands the
    icons in the same absolute spot while the bar's background fills the strip below) but it only holds if
    the viewport really does become full-bleed. On iOS it didn't, so the padding was pure added height and
    the icons rose by one home-indicator — on BOTH surfaces, including the installed PWA whose footer was
    already correct. In-browser there is a strip below the bar where page content shows through; that is a
    separate Safari 26 issue and is NOT worth trading the bar's position for. Nothing in the app reads
    safe-area insets, deliberately. Verify any change here on a real device before shipping — the geometry
    that matters is not reproducible in a headless browser.
